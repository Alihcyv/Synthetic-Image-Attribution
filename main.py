...
from src.model import SpectralHELIX_V2
from src.inference import run_inference
from src.dataset import prepare_dataloaders
from src.trainer import EMA, run_epoch, evaluate_model


def train_one_fold(fold: int, train_df: pd.DataFrame, val_df: pd.DataFrame, cfg: Any) -> float:
    from src.dataset import get_transforms, ImageDataset
...
def main():
    ...
    print(f"\nOverall CV Accuracy: {np.mean(fold_results):.4f}")
    run_inference(CFG, SpectralHELIX_V2)
